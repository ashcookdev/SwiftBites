...
export default function DriversUpdateForm(props) {
  const {
    id: idProp,
    drivers: driversModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = driversRecord
      ? { ...initialValues, ...driversRecord }
      : initialValues;
    ...
    setCarReg(cleanValues.CarReg);
    setMOTDate(cleanValues.MOTDate);
    setStartDate(cleanValues.StartDate);
    setBasicWage(cleanValues.BasicWage);
    setStarRating(cleanValues.StarRating);
    setInsuranceDetails(
      typeof cleanValues.InsuranceDetails === "string" ||
        cleanValues.InsuranceDetails === null
        ? cleanValues.InsuranceDetails
        : JSON.stringify(cleanValues.InsuranceDetails)
    );
    setInterviewNotes(cleanValues.InterviewNotes);
    setNationality(cleanValues.Nationality);
    setYearsDriving(cleanValues.YearsDriving);
    setDeliverys(cleanValues.Deliverys);
    setContractFinish(cleanValues.ContractFinish);
    setInternalReviews(cleanValues.InternalReviews);
    setContractType(cleanValues.ContractType);
    setArea(cleanValues.Area);
    setNationalInsurance(cleanValues.NationalInsurance);
    setTelephone(cleanValues.Telephone);
    setLicenceNumber(cleanValues.LicenceNumber);
    setPassedInterview(cleanValues.PassedInterview);
    setDriverReviews(
      typeof cleanValues.DriverReviews === "string" ||
        cleanValues.DriverReviews === null
        ? cleanValues.DriverReviews
        : JSON.stringify(cleanValues.DriverReviews)
    );
    setDOB(cleanValues.DOB);
    setInterviewAccepted(cleanValues.InterviewAccepted);
    setHRnotes(cleanValues.HRnotes);
    setCurrent(cleanValues.Current);
    setCounty(cleanValues.County);
    setCountry(cleanValues.Country);
    setErrors({});
  };
  const [driversRecord, setDriversRecord] = React.useState(driversModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Drivers, idProp)
        : driversModelProp;
      setDriversRecord(record);
    };
    queryData();
  }, [idProp, driversModelProp]);
  React.useEffect(resetStateValues, [driversRecord]);
  const validations = {
    Name: [],
    Address: [],
    PostCode: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Drivers.copyOf(driversRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "DriversUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || driversModelProp)}
