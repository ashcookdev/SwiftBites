...
import { updateMenuItems } from "../mutations";
const client = generateClient();
export default function MenuItemsUpdateForm(props) {
  const {
    id: idProp,
    menuItems: menuItemsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = menuItemsRecord
      ? { ...initialValues, ...menuItemsRecord }
      : initialValues;
    setName(cleanValues.Name);
    setPriceDelivery(cleanValues.PriceDelivery);
    ...
    setPriceCollection(cleanValues.PriceCollection);
    setErrors({});
  };
  const [menuItemsRecord, setMenuItemsRecord] =
    React.useState(menuItemsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getMenuItems.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getMenuItems
        : menuItemsModelProp;
      setMenuItemsRecord(record);
    };
    queryData();
  }, [idProp, menuItemsModelProp]);
  React.useEffect(resetStateValues, [menuItemsRecord]);
  const validations = {
    Name: [],
    PriceDelivery: [],
    StockLevel: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateMenuItems.replaceAll("__typename", ""),
            variables: {
              input: {
                id: menuItemsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "MenuItemsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || menuItemsModelProp)}
