...
export default function ResturantInvoicUpdateForm(props) {
  const {
    id: idProp,
    resturantInvoic: resturantInvoicModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = resturantInvoicRecord
      ? { ...initialValues, ...resturantInvoicRecord }
      : initialValues;
    setMondayDrivers(
  ...
  const [resturantInvoicRecord, setResturantInvoicRecord] = React.useState(
    resturantInvoicModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(ResturantInvoic, idProp)
        : resturantInvoicModelProp;
      setResturantInvoicRecord(record);
    };
    queryData();
  }, [idProp, resturantInvoicModelProp]);
  React.useEffect(resetStateValues, [resturantInvoicRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            ResturantInvoic.copyOf(resturantInvoicRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ResturantInvoicUpdateForm")}
      {...rest}
    >
      <TextAreaField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || resturantInvoicModelProp)}
