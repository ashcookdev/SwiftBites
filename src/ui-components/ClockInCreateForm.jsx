...
export default function ClockInCreateForm(props) {
  ...
  const resetStateValues = () => {
    setClockIn(initialValues.ClockIn);
    setClockOut(initialValues.ClockOut);
    setSick(initialValues.Sick);
    setHoliday(initialValues.Holiday);
    setStaffId(initialValues.StaffId);
    setTimeEntryId(initialValues.TimeEntryId);
    setDate(initialValues.Date);
    setClockedIn(initialValues.ClockedIn);
    setClockedOut(initialValues.ClockedOut);
    setStaffName(initialValues.StaffName);
    setStaffRole(initialValues.StaffRole);
    setBreak1(initialValues.Break);
    setBreakStart(initialValues.BreakStart);
    setBreakEnd(initialValues.BreakEnd);
    setErrors({});
  };
  const validations = {
    ClockIn: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(new ClockIn0(modelFields));
          if (onSuccess) {
      ...
      {...getOverrideProps(overrides, "ClockInCreateForm")}
      {...rest}
    >
      <TextField
        label="Clock in"
